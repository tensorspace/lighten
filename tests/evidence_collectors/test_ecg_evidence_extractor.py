...
class TestECGEvidenceExtractor(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.extractor = ECGEvidenceExtractor(
    ...
    def test_regex_extraction(self):
        ...
        ecg_notes = pd.DataFrame(
            {
                ...
                "text": [
                    "ECG shows new ST segment elevation in leads V2, V3. Also T wave inversion."
                ],
            }
        )
        ...
        self.assertTrue(
            any(f["finding"] == "ST Elevation" for f in evidence["ecg_findings"])
        )
        self.assertTrue(
            any(f["finding"] == "T Wave Inversion" for f in evidence["ecg_findings"])
        )

    @patch(
        "lighten_ml.evidence_collectors.ecg_evidence_extractor.ECGEvidenceExtractor._post_process_llm_findings"
    )
    ...
    def test_llm_post_validation_contiguous_leads(self):
        ...
        processed = self.extractor._post_process_llm_findings(
            [finding_valid, finding_invalid]
        )
    ...
    def test_llm_post_validation_is_new_flag(self):
        ...
        finding_valid = {
            ...
            "is_new": True,
            "leads": ["V1", "V2"],
            "measurements": {"t_wave_inversion_mm": 2.0},
        }
        finding_invalid = {
            "finding": "T Wave Inversion",
            "is_new": False,
    ...
    def test_patient_centric_logging_in_regex_mode(self):
        ...
        patient_id = "log_ecg_1"
        hadm_id = "log_adm_ecg_1"
        notes = pd.DataFrame({"text": ["ST elevation noted."]})

        self.mock_notes_loader.get_patient_notes.return_value = notes
        self.mock_llm_client.enabled = False

        with self.assertLogs('lighten_ml.evidence_collectors.ecg_evidence_extractor', level='INFO') as cm:
    ...
    def test_patient_centric_logging_in_llm_mode(self):
        ...
        patient_id = "log_ecg_2"
        hadm_id = "log_adm_ecg_2"
        notes = pd.DataFrame({"text": ["ST elevation noted."]})
        llm_output = {"ecg_findings": []}
