...
def test_mi_onset_date_extraction():
            ...
            "troponin_data": [
                {
                    "value": 0.08,
                    "unit": "ng/mL",
                    "timestamp": "3/14/2024 06:30",
                    "above_threshold": True,
                },
                {
                    "value": 2.4,
                    "unit": "ng/mL",
                    "timestamp": "3/14/2024 12:00",
                    "above_threshold": True,
                },
            ],
            ...
            "troponin_data": [
                {
                    "value": 3.2,
                    "unit": "ng/mL",
                    "timestamp": "3/12/2024 15:00",
                    "above_threshold": True,
                },
                {
                    "value": 2.8,
                    "unit": "ng/mL",
                    "timestamp": "3/12/2024 21:00",
                    "above_threshold": True,
                },
            ],
    ...
    logger.info(
        f"[STATS] Perfect Matches: {perfect_matches}/{total_scenarios} ({perfect_matches/total_scenarios*100:.1f}%)"
    )
    logger.info(
        f"[STATS] Date Matches: {date_matches}/{total_scenarios} ({date_matches/total_scenarios*100:.1f}%)"
    )
    ...
    for result in results:
        if not result.get("error"):
            match_status = (
                ...
                else "[WARNING] PARTIAL" if result.get("date_match") else "[ERROR] MISS"
            )
            logger.info(
                f"{match_status} - Scenario {result['scenario']}: {result['name']}"
            )
            logger.info(
                f"   [DATE] Date: {result.get('extracted_date')} (expected: {result.get('expected_date')})"
            )
            logger.info(
                f"   [TARGET] Basis: {result.get('selection_basis')} (expected: {result.get('expected_basis')})"
            )
            logger.info(f"   [STATS] Confidence: {result.get('confidence', 0.0):.2f}")
        else:
            logger.info(
                f"[ERROR] ERROR - Scenario {result['scenario']}: {result.get('error')}"
            )
    ...
    logger.info(
        "[COMPLETE] This system can intelligently process clinical documentation!"
    )
    logger.info("=" * 80)
