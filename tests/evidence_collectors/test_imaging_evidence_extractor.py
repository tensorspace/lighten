...
from lighten_ml.evidence_collectors.imaging_evidence_extractor import ImagingEvidenceExtractor
...
class TestImagingEvidenceExtractor(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.extractor = ImagingEvidenceExtractor(self.mock_notes_loader, llm_client=self.mock_llm_client)
    ...
    def test_regex_extraction(self):
        ...
        notes = pd.DataFrame({
            ...
            'text': ['Echocardiogram shows a new wall motion abnormality in the anterior region.']
        })
        ...
        self.assertEqual(evidence['imaging_findings'][0]['finding'], 'Wall Motion Abnormality')
    ...
    def test_llm_extraction(self):
        ...
        notes = pd.DataFrame({
            ...
            'text': ['Echo reveals severe hypokinesis of the apex. This is an acute finding.']
        })

        llm_output = {
            'imaging_findings': [
                {'finding': 'Wall Motion Abnormality', 'context': 'hypokinesis of the apex', 'is_new': True}
            ]
        }
