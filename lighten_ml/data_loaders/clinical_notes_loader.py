...
class ClinicalNotesLoader(BaseDataLoader):
    ...
    def search_notes_for_keywords(
        self, patient_id: str, hadm_id: str, keywords: List[str]
    ) -> List[Dict[str, Any]]:
            ...
            for keyword in keywords:
                if keyword.lower() in text:
                    ...
                    start = max(0, text.find(keyword.lower()) - 100)
                        ...
                        len(text), text.find(keyword.lower()) + len(keyword) + 100
                    )
    ...
    def get_patient_data(
        ...
        """
        notes = self.get_patient_notes(patient_id, hadm_id)
        return {"notes": notes.to_dict("records") if not notes.empty else []}
