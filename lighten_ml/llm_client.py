        ...
        for attempt in range(self.retries):
            try:
                ...
                try:
                    ...
                    logger.info(
                        f"LLM PARSED JSON - Keys: {list(parsed_json.keys()) if isinstance(parsed_json, dict) else 'Not a dict'}"
                    )
                    if isinstance(parsed_json, dict):
                        for key, value in parsed_json.items():
                            if isinstance(value, list):
                                logger.info(
                                    f"LLM PARSED JSON - {key}: {len(value)} items"
                                )
                            elif isinstance(value, dict):
                                logger.info(
                                    f"LLM PARSED JSON - {key}: dict with {len(value)} keys"
                                )
                            else:
                                logger.info(
                                    f"LLM PARSED JSON - {key}: {type(value).__name__}"
                                )
