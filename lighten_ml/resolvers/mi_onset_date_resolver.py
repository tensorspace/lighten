...
class MIOnsetDateResolver:
    ...
    def extract_mi_onset_date(
        ...
        """
        logger.info(f"[{hadm_id}] [EXTRACT] MI ONSET DATE EXTRACTION - Starting LLM analysis")
        ...
        try:
            ...
            logger.info(f"[{hadm_id}] [DATE] Final MI Onset Date: {final_result.get('onset_date', 'Not determined')}")
            logger.info(f"[{hadm_id}] [BASIS] Selection Basis: {final_result.get('selection_basis', 'N/A')}")
...
"""
        ...
        try:
            ...
            logger.info("[SUCCESS] LLM MI ONSET DATE EXTRACTION - JSON parsed successfully")
            
            return result
            
        except json.JSONDecodeError as e:
            logger.error(f"[ERROR] LLM MI ONSET DATE EXTRACTION - JSON parsing failed: {e}")
    ...
    def _prepare_troponin_summary(self, troponin_data: List[Dict]) -> str:
        ...
        for i, test in enumerate(troponin_data[:10]):  # Limit to first 10 tests
            ...
            threshold_status = "ABOVE THRESHOLD" if above_threshold else "below threshold"
    ...
    def _process_llm_results(
        ...
        logger.info(f"[{hadm_id}] [PROCESS] PROCESSING LLM RESULTS - Validating extracted dates")
        ...
        for level_name, priority in self.DATE_HIERARCHY.items():
            ...
            if level_data and level_data.get('date'):
                ...
                if validated_date:
                    ...
                    logger.info(f"[{hadm_id}] [VALID] {level_name.upper()}: {validated_date} (confidence: {level_data.get('confidence', 0.0)})")
                else:
                    processed["validation_notes"].append(f"Invalid date format for {level_name}: {level_data.get('date')}")
                    logger.warning(f"[{hadm_id}] [WARNING] {level_name.upper()}: Invalid date format - {level_data.get('date')}")
            else:
    ...
    def _validate_and_parse_date(self, date_str: str, hadm_id: str, level_name: str) -> Optional[str]:
        ...
        logger.warning(f"[{hadm_id}] [WARNING] DATE VALIDATION FAILED - {level_name}: {date_str}")
        return None

    def _apply_guideline_hierarchy(self, processed_result: Dict[str, Any], hadm_id: str) -> Dict[str, Any]:
        ...
        if not hierarchy_analysis:
            logger.warning(f"[{hadm_id}] [WARNING] No valid dates found in hierarchy analysis")
            return {
                "onset_date": None,
        ...
        if selected_level and selected_data:
            logger.info(f"[{hadm_id}] [SELECTED] SELECTED DATE: {selected_data['date']} from {selected_level}")
            logger.info(f"[{hadm_id}] [EVIDENCE] EVIDENCE: {selected_data.get('evidence', 'N/A')}")
            ...
            logger.warning(f"[{hadm_id}] [WARNING] No dates met minimum confidence threshold (0.5)")
            return {
                "onset_date": None,
