    ...
    try:
        ...
        try:
            ...
            if lab_loader.data is not None and not lab_loader.data.empty:
                ...
                available_columns = [
                    col for col in critical_columns if col in lab_loader.data.columns
                ]
                missing_columns = [
                    col
                ...
                for idx, row in sample_data.iterrows():
                    logger.info(
                        f"  Row {idx}: itemid={row.get('itemid', 'N/A')}, "
        ...
        try:
            ...
            if lab_loader.data is not None and not lab_loader.data.empty:
                ...
                logger.info(
                    f"Testing enhanced troponin analysis for patient {sample_patient}, admission {sample_hadm}"
                )

                # This should now properly handle units and time columns
                troponin_evidence = troponin_analyzer.collect_evidence(
                ...
                logger.info(
                    f"MI criteria met: {troponin_evidence.get('mi_criteria_met', False)}"
                )
                logger.info(
                    f"Max troponin: {troponin_evidence.get('max_troponin', 'N/A')}"
                )
                logger.info(
                    f"Number of troponin tests: {len(troponin_evidence.get('troponin_tests', []))}"
                )
