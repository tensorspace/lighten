...
def test_simplified_lab_identification():
    ...
    try:
        ...
        if lab_loader._itemid_to_label_map:
            logger.info(f"✅ Itemid mapping initialized: {len(lab_loader._itemid_to_label_map)} items")
            
            # Show some example mappings
            example_itemids = [51003, 52642, 51002, 50809, 50931]  # Troponin and glucose examples
        ...
        for itemid in glucose_itemids[:5]:  # Show first 5
            info = lab_loader.get_lab_test_info(itemid)
            if info:
                logger.info(f"  Glucose {itemid}: {info['label']} (fluid: {info['fluid']})")
        ...
        try:
            ...
            if lab_loader.data is not None and not lab_loader.data.empty:
                ...
                logger.info(f"✅ Troponin tests found (simplified): {len(troponin_tests)}")
                
                # Test direct itemid retrieval for glucose
                if glucose_itemids:
                    glucose_tests = lab_loader.get_lab_tests_by_itemids(
                        sample_patient, sample_hadm, glucose_itemids[:3]  # Test first 3 glucose types
                    )
                    logger.info(f"✅ Glucose tests found (direct itemid): {len(glucose_tests)}")
                
                # Test legacy method compatibility
                legacy_glucose = lab_loader.get_lab_tests_by_type(sample_patient, sample_hadm, "glucose")
                logger.info(f"✅ Legacy glucose search (now uses itemids): {len(legacy_glucose)}")
        ...
        logger.info("1. labevents.csv has: patient_id, hadm_id, itemid=50809, valuenum=120")
        logger.info("2. d_labitems.csv maps: itemid=50809 -> 'Glucose' (fluid='Blood')")
        logger.info("3. Pipeline gets: Glucose=120 with proper label for downstream processing")
        ...
        if glucose_itemids:
            for itemid in glucose_itemids[:3]:
                info = lab_loader.get_lab_test_info(itemid)
                if info:
                    logger.info(f"   - itemid={itemid}: {info['label']} (fluid={info['fluid']})")
