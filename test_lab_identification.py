...
def test_lab_identification():
    ...
    try:
        ...
        if lab_loader.data is not None and not lab_loader.data.empty:
            ...
            logger.info(
                f"Mapped itemids present in data: {len(present_itemids)} out of {len(all_mapped_itemids)}"
            )
            ...
            if len(lab_loader.data) > 0:
                ...
                for test_type in ["glucose", "creatinine", "hemoglobin"]:
                    if test_type in lab_loader.LAB_TEST_ITEMIDS:
                        results = lab_loader.get_lab_tests_by_type(
                            sample_patient, sample_hadm, test_type
                        )
                        logger.info(f"{test_type.title()} tests found: {len(results)}")

                # Test comprehensive summary
                summary = lab_loader.get_comprehensive_lab_summary(
                    sample_patient, sample_hadm
                )
                logger.info(f"Comprehensive lab summary generated for {sample_patient}")
                logger.info(
                    f"Total tests across all categories: {sum(summary['test_counts'].values())}"
                )
        else:
